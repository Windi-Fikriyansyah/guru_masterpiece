...
@section('header', 'Generator Ice Breaking')
...
<div class="max-w-5xl mx-auto space-y-10">
    ...
    <div class="text-center">
        <h2 class="text-3xl font-extrabold text-slate-800 mb-2">Generator Ice Breaking</h2>
        <p class="text-slate-500">Temukan ide permainan seru untuk mencairkan suasana kelas dalam sekejap.</p>
    </div>
    ...
    <div class="bg-white rounded-[2rem] p-8 lg:p-10 shadow-xl shadow-slate-200/60 border border-slate-100">
        <div class="flex items-center gap-3 mb-8 border-b border-slate-100 pb-4">
            ...
            <div>
                <h3 class="text-xl font-bold text-slate-800">Konfigurasi Ice Breaking</h3>
                <p class="text-sm text-slate-400">Sesuaikan permainan dengan kondisi kelas Anda</p>
            </div>
        </div>
        
        <form id="iceBreakingForm" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <!-- Tipe Permainan -->
                <div class="space-y-2">
                    <label for="tipe_game" class="block text-sm font-bold text-slate-700 ml-1">Tipe Permainan</label>
                    <select name="tipe_game" id="tipe_game" class="w-full select2-enable" required>
                        <option value="">Pilih Tipe</option>
                        <option value="Indoor (Dalam Kelas)">Indoor (Dalam Kelas)</option>
                        <option value="Outdoor (Luar Kelas)">Outdoor (Luar Kelas)</option>
                        <option value="Energizer / Fokus">Energizer / Fokus</option>
                        <option value="Team Building (Kelompok)">Team Building (Kelompok)</option>
                        <option value="Tanpa Alat">Tanpa Alat</option>
                        <option value="Brain Gym / Senam Otak">Brain Gym / Senam Otak</option>
                    </select>
                </div>

                 <!-- Estimasi Jumlah Siswa -->
                 <div class="space-y-2">
                    <label for="jumlah_siswa" class="block text-sm font-bold text-slate-700 ml-1">Estimasi Jumlah Siswa</label>
                    <input type="number" name="jumlah_siswa" id="jumlah_siswa" class="w-full bg-slate-50 border border-slate-300 rounded-xl px-4 py-3 focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all font-medium text-slate-700 placeholder:text-slate-400" placeholder="Contoh: 30" required>
                </div>
            </div>

            <!-- Tujuan & Instruksi -->
            <div class="space-y-2">
                <label for="tujuan" class="block text-sm font-bold text-slate-700 ml-1">Tujuan Ice Breaking</label>
                <input type="text" name="tujuan" id="tujuan" class="w-full bg-slate-50 border border-slate-300 rounded-xl px-4 py-3 focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all font-medium text-slate-700 placeholder:text-slate-400" placeholder="Contoh: Meningkatkan kefokusan, perkenalan, atau transisi mapel" required>
            </div>

            <div class="space-y-2">
                <label for="instruksi_khusus" class="block text-sm font-bold text-slate-700 ml-1">Instruksi / Keinginan Khusus</label>
                <textarea name="instruksi_khusus" id="instruksi_khusus" rows="3" class="w-full bg-slate-50 border border-slate-300 rounded-xl px-4 py-3 focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all font-medium text-slate-700 placeholder:text-slate-400" placeholder="Contoh: permainan singkat 5 menit, melibatkan gerakan fisik, dll"></textarea>
            </div>
    ...
    <div id="resultSection" class="space-y-6">
        <div class="flex items-center gap-4">
            ...
            <h3 class="text-lg font-bold text-slate-400 uppercase tracking-widest">Ide Ice Breaking</h3>
            <div class="h-[1px] flex-1 bg-slate-200"></div>
        </div>
        ...
        <div id="placeholderResult" class="bg-white rounded-[2rem] border-2 border-dashed border-slate-200 p-12 text-center min-h-[300px] flex flex-col items-center justify-center">
            ...
            <h4 class="text-xl font-bold text-slate-400 mb-2">Belum Ada Ide</h4>
            <p class="text-slate-400 max-w-sm">Isi parameter di atas dan klik tombol cari ide untuk menemukan permainan seru.</p>
        </div>
        ...
        <div id="loadingResult" class="hidden bg-white rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/60 p-12 text-center min-h-[300px] flex flex-col items-center justify-center">
            ...
            <h4 class="text-xl font-bold text-slate-600 mb-2">Sedang Mencari Ide...</h4>
            <p class="text-slate-400 max-w-sm">AI sedang memikirkan permainan yang cocok untuk kelas Anda.</p>
        </div>
...
<script>
                ...
                success: function(response) {
                    if(response.result) {
                        ...
                        Swal.fire({
                            icon: 'success',
                            title: 'Ide Ditemukan!',
                            text: 'Beberapa opsi ice breaking telah disiapkan.',
    ...
    function downloadPDF() {
        ...
        const opt = {
            margin: [15, 15, 15, 15],
            filename: 'Ice-Breaking-Ideas.pdf',
    ...
    function downloadWord() {
        ...
        link.download = 'Ice-Breaking-Ideas.docx';
        link.click();
